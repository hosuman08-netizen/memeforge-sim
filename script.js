...
/* WAVE92: peak-now 갭칩. sim momPeak−now only. no invented cap · no live X · no live pay. */
function momGapChip(c) {
  var v = +(c && c.momentum);
  if (!isFinite(v)) v = computeMomentum(c || {});
  var p = +(c && c.momPeak);
  if (!isFinite(p) || v > p) p = v;
...
/* WAVE115: 피크경과칩. sim now−momPeakAt only. no invented cap · no live X · no live pay. */
function momPeakAgoChip(c) {
  var v = +(c && c.momentum);
  if (!isFinite(v)) v = computeMomentum(c || {});
  var p = +(c && c.momPeak);
  var t = +(c && c.momPeakAt);
  ...
  if (isFinite(t) && t > 0) {
    ...
    if (sec < 60) label = '경과 ' + sec + 's';
    else if (sec < 3600) label = '경과 ' + Math.floor(sec / 60) + 'm';
    else label = '경과 ' + Math.floor(sec / 3600) + 'h';
  }
...
/* WAVE195: 끈 뒤 바 포커스 유지. sim only. no invented cap. */
/* WAVE199: 포커스 링 재탭=재시작 분리. sim only. no invented cap. */
...
function momBarFlashOn(el) {
  ...
  if (el.classList && el.classList.contains('mom-flash')) return true;
...
function restartMomBarRingFromFocus() {
  var el = typeof document !== 'undefined' ? document.getElementById(momPeakBackAgoJumpId()) : null;
